...
class TrainingSessionServiceTest {
    ...
    @Test
    void addPlayerToTrainingSessionSuccess() {
        when(trainingSessionRepo.findById(1L)).thenReturn(Optional.of(session));
        when(userRepo.findByUsernameIgnoreCase("player1")).thenReturn(Optional.of(player1));
        when(trainingSessionRepo.save(any(TrainingSession.class))).thenAnswer(inv -> inv.getArgument(0));

        TrainingSessionResponse saved = service.addPlayerToTrainingSession("player1", 1L);
        ...
        assertTrue(session.getPlayers().contains(player1));
        verify(trainingSessionRepo).save(session);
    }

    @ParameterizedTest
    @MethodSource("invalidAddPlayerInputs")
    void addPlayerToTrainingSessionInvalidInputs(String username, Long trainingSessionId, String expectedMessage) {
        IllegalArgumentException ex = assertThrows(
                IllegalArgumentException.class,
                () -> service.addPlayerToTrainingSession(username, trainingSessionId)
        );

        assertEquals(expectedMessage, ex.getMessage());
    }

    private static Stream<Arguments> invalidAddPlayerInputs() {
        return Stream.of(
                Arguments.of(null, 1L, "Username is required"),
                Arguments.of("", 1L, "Username is required"),
                Arguments.of("   ", 1L, "Username is required"),
                Arguments.of("player1", null, "Training session id is required")
        );
    }

    @Test
    void addPlayerToTrainingSessionSessionNotFound() {
        when(trainingSessionRepo.findById(1L)).thenReturn(Optional.empty());

        IllegalArgumentException ex = assertThrows(
                IllegalArgumentException.class,
                () -> service.addPlayerToTrainingSession("player1", 1L)
        );

        assertEquals("Training session [id=1] not found", ex.getMessage());
    }

    @Test
    void addPlayerToTrainingSessionPlayerNotFound() {
        when(trainingSessionRepo.findById(1L)).thenReturn(Optional.of(session));
        when(userRepo.findByUsernameIgnoreCase("player1")).thenReturn(Optional.empty());
        ...
        assertEquals("Player: player1 not found", ex.getMessage());
    }

    @Test
    void addPlayerToTrainingSessionUserNotPlayer() {
        when(trainingSessionRepo.findById(1L)).thenReturn(Optional.of(session));
        when(userRepo.findByUsernameIgnoreCase("coach1")).thenReturn(Optional.of(coach));

        IllegalArgumentException ex = assertThrows(
                IllegalArgumentException.class,
                () -> service.addPlayerToTrainingSession("coach1", 1L)
        );

        assertEquals("User: coach1 is not a player", ex.getMessage());
    }

    @Test
    void addPlayerToTrainingSessionAlreadyAdded() {
        session.getPlayers().add(player1);
        ...
        assertEquals("Player: player1 already added to training session", ex.getMessage());
    }
